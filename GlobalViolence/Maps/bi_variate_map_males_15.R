...
GBD_GPI_male$continent <- countrycode(sourcevar = GBD_GPI_male[, "country"],
                                      origin = "country.name",
...
map <- ggplot()+
  geom_sf(data = data_map_men,  mapping = aes(fill = bi_class),
          color = "white", size = 0.1, show.legend = T) +
  bi_scale_fill(pal = "DkViolet", dim = 3)+ geom_text(x=3, y=30, label="Scatter plot")+
  theme_map()
